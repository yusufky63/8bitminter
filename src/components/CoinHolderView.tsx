...
import {
  getProfileBalance,
  getZoraProfile,
} from "../services/sdk/getProfiles.js";
...
export default function CoinHolderView() {
  ...
  const [sortBy, setSortBy] = useState<
    "balance" | "marketCap" | "holders" | "volume" | "name" | "usdValue"
  >("usdValue");
    ...
    if (typeof window !== "undefined") {
      ...
      const handleViewCoinDetails = (event: CustomEvent) => {
        if (event.detail && event.detail.tokenAddress) {
          console.log(
            "Received viewCoinDetails event:",
            event.detail.tokenAddress
          );
      ...
      window.addEventListener(
        "viewCoinDetails",
        handleViewCoinDetails as EventListener
      );
  ...
  const fetchTokenBalances = async (walletAddress: string) => {
    setIsLoading(true);
    try {
      const response = (await getProfileBalance(
        walletAddress
      )) as ProfileBalanceResponse;

      if (response?.data?.profile?.coinBalances?.edges) {
        const balances = response.data.profile.coinBalances.edges
          .filter(
            (edge) =>
              edge.node && edge.node.coin && parseFloat(edge.node.balance) > 0
          )
          .map((edge) => {
            return {
  ...
  return (
    <div className="retro-container p-2 border-2 border-retro-primary">
      ...
      <div className="mb-2 bg-gradient-to-r from-retro-primary/20 to-retro-primary/5 p-1 border-2 border-retro-primary shadow-[0_0_10px_rgba(255,107,53,0.1)]">
        ...
        {isConnected ? (
          <>
            {isProfileLoading ? (
              ...
              <div className="text-center py-2 bg-gradient-to-br from-red-900/20 to-black/40 border-2 border-red-400">
                <p className="text-red-400 text-xs mb-3 pixelated">
                  {profileError}
                </p>
                <RetroButton
              ...
              <div className="bg-gradient-to-br from-retro-primary/10 via-retro-primary/5 to-black/40 p-4 border-2 border-retro-primary shadow-[0_0_15px_rgba(255,107,53,0.2)]">
                <div className="flex items-center gap-4">
                  {userProfile.avatar && resolveImageUrl(userProfile.avatar) ? (
                    <Image
                      ...
                      onError={(e) => {
                        ...
                        if (parent) {
                          parent.innerHTML =
                            '<div class="w-14 h-14 rounded-lg bg-retro-primary/20 border-2 border-retro-primary flex items-center justify-center shadow-[0_0_8px_rgba(255,107,53,0.3)]"><span class="text-retro-primary text-xl font-bold pixelated">ğŸ“·</span></div>';
                        }
                      }}
                    />
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    {userProfile.handle && (
                      <div className="text-sm text-retro-secondary mb-2 font-mono">
                        @{userProfile.handle}
                      </div>
                    )}
              ...
              <div className="text-center py-4 border-2 border-retro-primary bg-black/20">
                <p className="text-xs text-retro-secondary pixelated">
          ...
          <div className="text-center py-6 border-2 border-retro-primary bg-gradient-to-br from-retro-primary/5 to-black/40">
            ...
            <p className="text-sm text-retro-primary mb-2 pixelated font-bold">
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 relative ">
          ...
          <svg
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            className="absolute right-2 top-1/2 transform -translate-y-1/2 text-retro-secondary"
          >
      ...
      {isConnected && allTokens.length > 0 && (
        <div className="mb-3 bg-black/20 p-2 border border-retro-primary">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-xs text-retro-secondary pixelated">
              SORT BY:
            </span>
      ...
      {isLoading ? (
            ...
            {tokens.map((t) => {
              ...
              return (
                <RetroTokenCard
                  key={t.address}
                  token={mapped}
                  onClick={(addr) => handleTokenClick(addr)}
        ...
        <div className="text-center py-8 border-2 border-retro-primary bg-gradient-to-br from-retro-primary/5 to-black/40">
          ...
          <p className="text-retro-primary mb-2 pixelated font-bold">
            NO TOKENS FOUND
          </p>
          <p className="text-xs text-retro-secondary">
            {isConnected
