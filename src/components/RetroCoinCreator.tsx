...
import {
  detectEnvironment,
  getPreferredConnectorId,
  isWalletAvailable,
  getWalletAvailabilityMessage,
...
export default function RetroCoinCreator() {
  ...
  const [autoCreateAfterConnect, setAutoCreateAfterConnect] =
  ...
  const uiIsConnected =
    isConnected || appEnv === "farcaster" || appEnv === "baseapp";
  ...
  const simplifyErrorMessage = (raw: string) => {
    ...
    if (
      ...
      (msg.includes("switch") && msg.includes("chain")) ||
  ...
  useEffect(() => {
    ...
    try {
      if (
        ...
        const farcasterLike = connectors.find(
          (c) =>
  ...
  const generateAiSuggestions = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Success toast
        toast.success("Analysis complete! Token details generated", {
          id: "status-toast",
        });
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener(
        "ai-data-ready",
        handleAiDataReady as EventListener
      );
    }
    ...
    try {
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          if (attempts > 1) {
            toast.loading(
              `Retrying image generation (attempt ${attempts}/${maxAttempts})...`,
              {
                id: "status-toast",
              }
            );
          ...
          try {
            ...
            if (!ipfsResp.ok) {
              ...
              throw new Error(
                err.error || `IPFS upload route error: ${ipfsResp.status}`
              );
            }
          ...
          if (attempts < maxAttempts) {
            toast.error(`Attempt ${attempts} failed, retrying...`, {
              id: "status-toast",
            });
            ...
            toast.error(
              `Image generation failed after ${maxAttempts} attempts`,
              { id: "status-toast" }
            );
            setError(
              `Failed to generate image after ${maxAttempts} attempts: ${
                error instanceof Error ? error.message : "Unknown error"
              }`
            );
          }
  ...
  const handleCreateCoin = useCallback(async () => {
    ...
    try {
      ...
      if (chainId !== base.id) {
        console.log(
          `Currently on network ${chainId}, switching to Base network (${base.id})`
        );
        ...
        try {
          ...
          if (
            ...
            setError(
              "Network switch was rejected. Please manually switch to Base network in your wallet."
            );
          } else {
            setError(
              `Failed to switch to Base network automatically. Please manually switch to Base network (ID: ${base.id}) in your wallet and try again.`
            );
          }
      ...
      if (
        ...
        contractAddress =
          extractedAddress || "Contract created, address unknown";
      } else {
      ...
      if (
        ...
        try {
          ...
          if (savedCoin) {
            toast.success("Coin saved to database successfully!", {
              id: "save-toast",
            });
          } else {
            toast.error("Failed to save coin to database", {
              id: "save-toast",
            });
      ...
      // Set success message
      setSuccess(
        `Coin ${formData.name} (${formData.symbol}) created successfully on Base network!`
      );
  ...
  const connectWallet = async () => {
    try {
      ...
      if (!connectors || connectors.length === 0) {
        ...
        setError(
          "Wallet connection is not available. Please ensure you're using a supported app."
        );
        toast.error(
          "Wallet connection is not available. Please ensure you're using a supported app.",
          { id: "status-toast" }
      ...
      if (envForConnect === "farcaster" && !isTrustedFarcasterHost()) {
        const isEmbedded =
          typeof window !== "undefined" && (window as any).top !== window;
        if (isEmbedded) {
          const msg =
            "Wallet connect is blocked in this embedded view. Open the app in your browser or the official Farcaster app.";
          setError(msg);
      ...
      console.log(
        `Environment: ${environment}, Preferred connector: ${preferredConnectorId}`
      );
      ...
      if (preferredConnector) {
        ...
        console.log(
          `Using ${preferredConnector.name} connector for ${environment}`
        );
      } else {
        console.log(
          `Preferred connector ${preferredConnectorId} not found, using default`
        );
      }

      // In Farcaster env but untrusted host (e.g., tunnel), force injected fallback
      if (environment === "farcaster" && !isTrustedFarcasterHost()) {
        const injectedFallback = connectors.find(
          (c) =>
        ...
        if (injectedFallback) {
          ...
          console.log(
            "Untrusted Farcaster host detected; falling back to injected connector"
          );
        }
      }
      ...
      if (environment === "farcaster" && !hasFarcasterRuntime()) {
        ...
        if (injectedFallback) {
          targetConnector = injectedFallback;
          console.log(
            "Farcaster runtime not detected; falling back to injected connector"
          );
        }
      }
      ...
      try {
        ...
        const isOriginMismatch =
          msg.includes("origin") || msg.includes("origins don't match");
        if (isOriginMismatch) {
          const injectedFallback = connectors.find(
            (c) =>
              (c.id?.toLowerCase?.() || "").includes("injected") ||
          ...
          if (injectedFallback) {
            console.warn(
              "Connector origin mismatch; retrying with injected connector"
            );
      ...
      if (
        ...
        errorMessage.includes("No connector")
      ) {
        switch (environment) {
          case "baseapp":
            userMessage =
              "BaseApp wallet not available. Please ensure you're using the latest version of BaseApp.";
            break;
          case "farcaster":
            userMessage =
              "Farcaster wallet not available. Please ensure you're using Farcaster app or try a browser wallet.";
            break;
          case "browser":
            userMessage =
          ...
          default:
            userMessage =
              "Wallet not available in this environment. Please use BaseApp, Farcaster, or a browser with wallet extension.";
        }
      } else if (
        ...
        userMessage =
          "Please switch to Base network in your wallet and try again.";
      } else {
    ...
    const initFarcasterSDK = async () => {
      try {
        ...
        try {
          ...
          const sdkModule = await import("@farcaster/frame-sdk/dist").catch(
            (e) => null
          );
          ...
          // YaklaÅŸÄ±m 1 baÅŸarÄ±sÄ±z olduysa 2. yaklaÅŸÄ±mÄ± dene
            ...
            const fallbackModule = await import("@farcaster/frame-sdk").catch(
              (e) => null
            );
            ...
            if (fallbackModule && fallbackModule.sdk) {
              ...
              console.warn(
                "âš ï¸ Could not load Farcaster SDK - continuing without it"
              );
          ...
          if (farcasterSDK.current?.actions?.ready) {
            ...
            try {
              ...
              if (
                (envNow === "farcaster" || envNow === "baseapp") &&
                !isConnected &&
                connectors &&
                connectors.length > 0
              ) {
          ...
          console.warn(
            "SDK import failed - continuing without SDK:",
            importError
          );
          return; // SDK olmadan devam et
        }
  ...
  const steps = ["DEFINE", "CUSTOMIZE", "VISUALIZE", "CREATE"];
  ...
  return (
    <div className="w-full">
      {error && (
        <RetroNotification message={error} type="error" className="mb-4" />
      )}
      {success && (
        <RetroNotification message={success} type="success" className="mb-4" />
      )}
