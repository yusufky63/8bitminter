...
function generateFallbackSuggestion(category: string, description: string) {
  ...
  const symbol = (word1.substring(0, 2) + word2.substring(0, 2)).toUpperCase();

  // Simple description
  const desc = `A ${category.toLowerCase()} token focused on ${description.substring(
    0,
    50
  )}...`;
  ...
  try {
    ...
    if (action === "text") {
      ...
      try {
        ...
        console.log("🔄 Starting AI text generation with:", {
          category,
          description,
        });
          ...
          return NextResponse.json(
            {
              ...
              symbol: "RTK",
              description:
                "A retro-styled cryptocurrency token with unique features.",
              category: category,
              isFallback: true,
            },
            { headers: baseHeaders }
      ...
      try {
        ...
        console.log("🔄 Starting image generation with parameters:", {
          name,
          symbol,
          description,
        });

        // Generate image with AI - timeout kontrolü kaldırıldı
