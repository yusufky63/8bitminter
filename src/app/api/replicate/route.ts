...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, version, input } = body;
    ...
    // Default model version ve parametreler
    const defaultVersion = "db21e45d3f7023abc2a46ee38a23973f6dce16bb082a930b0c49861f96d1e5bf";
    const defaultInput = {
      prompt: prompt,
      num_outputs: 1,
      scheduler: "K_EULER",
    ...
    const response = await fetch(REPLICATE_API_BASE_URL, {
      ...
      body: JSON.stringify({
        // Eğer client tarafından gönderilmişse o değerleri, yoksa varsayılanları kullan
